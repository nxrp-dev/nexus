...
import { NexusBuildCommandBuilder } from '../build/nexusBuildCommandBuilder';
import { FpcTaskDefinition, LazarusTaskDefinition, NexusBuildTaskDefinition } from '../providers/taskDefinitions';
import { ExtensionPaths } from '../services/extensionPaths';
import { BuildTaskTerminal } from '../terminal/buildTaskTerminal';
import { BuildMode, FPC_TASK_TYPE, LAZARUS_TASK_TYPE, NEXUS_TASK_TYPE } from './vscodeTaskTypes';
...
export class NexusBuildTask extends vscode.Task {
    private readonly commandBuilder: NexusBuildCommandBuilder;

    public constructor(cwd: string, name: string, taskDefinition: NexusBuildTaskDefinition, extensionPaths: ExtensionPaths) {
        super(
            taskDefinition,
            vscode.TaskScope.Workspace,
            `${name}`,
            NEXUS_TASK_TYPE,
            new FpcCustomExecution(async (): Promise<vscode.Pseudoterminal> => {
                const command = this.commandBuilder.createCommand(cwd, taskDefinition);
                return new BuildTaskTerminal(command);
            })
        );
        this.commandBuilder = new NexusBuildCommandBuilder(extensionPaths);
        this.group = vscode.TaskGroup.Build;
        this.problemMatchers = ['$fpc'];
    }
}
