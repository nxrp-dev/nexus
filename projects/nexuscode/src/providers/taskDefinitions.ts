...
export class NexusBuildTaskDefinition implements vscode.TaskDefinition {
	[name: string]: any;
	readonly type: string = 'nexus';
	project?: string;
	cwd?: string;
}

export type NexusTaskDefinition = FpcTaskDefinition | LazarusTaskDefinition | NexusBuildTaskDefinition;
...
export function isNexusBuildTaskDefinition(ADefinition: vscode.TaskDefinition | undefined): ADefinition is NexusBuildTaskDefinition {
	return !!ADefinition
		&& ADefinition.type === 'nexus'
		&& typeof ADefinition.project === 'string'
		&& ADefinition.project.length > 0;
}
